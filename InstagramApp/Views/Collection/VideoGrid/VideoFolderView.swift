                ...
                ScrollView {
                    LazyVGrid(columns: [GridItem(spacing: 8), GridItem(spacing: 8)], spacing: 12,content: {
                        ForEach(Array(zip(folder.reelsArray.indices, folder.reelsArray)), id: \.0) { index, reels in
                            VideoCover(videosViewModel: reelsViewModel, reels: reels, folder: folder)
                        }
                    })
                }
                .padding(16)
...
#Preview {
    NavigationStack {
        VideoFolderView(reelsViewModel: ReelsViewModel(), folder: ReelsFolderModel(reelsArray: [mockReelsResponse.data.reels], name: "Fuck"), folderIndex: 0)
